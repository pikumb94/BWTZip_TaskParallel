...
extern vector<vector <unsigned char>*> input, stage1, stage2, output;

void BWTwrapper(int* i, int* cycle) {
	if (*cycle % 2 == 0) {
		stage1[*i % NUM_BLOCKS_PER_STAGE] = input[*i];
		bwtzip::bwtStree(*input[*i]);
	}
		//stage1[*i % NUM_BLOCKS_PER_STAGE] = BWT(input[*i]);
	else {
		stage1[(*i % NUM_BLOCKS_PER_STAGE) + NUM_BLOCKS_PER_STAGE] = input[*i];
		bwtzip::bwtStree(*input[*i]);
	}
...
void MTF_RLEwrapper(int *i, int *cycle) {
	if (*cycle % 2 == 0) {
		...
		stage1[*i] = stage1[*i];
		bwtzip::mtf2(*stage1[*i]);
		stage2[*i] = stage1[*i];
		bwtzip::zleWheeler(*stage1[*i]);
	}
	else {
		...
		stage1[NUM_BLOCKS_PER_STAGE + *i] = stage1[NUM_BLOCKS_PER_STAGE + *i];
		bwtzip::mtf2(*stage1[NUM_BLOCKS_PER_STAGE + *i]);
		stage2[NUM_BLOCKS_PER_STAGE + *i] = stage1[NUM_BLOCKS_PER_STAGE + *i];
		bwtzip::zleWheeler(*stage1[NUM_BLOCKS_PER_STAGE + *i]);
		}
...
void ARIwrapper(int *i, int *cycle) {
	if (*cycle % 2 == 0) {
		output[*i + *cycle * NUM_BLOCKS_PER_STAGE] = stage2[*i];
		bwtzip::arith(*stage2[*i]);
	}
		//output[*i + *cycle * NUM_BLOCKS_PER_STAGE] = ARI(stage2[*i]);
	else {
		output[*i + *cycle * NUM_BLOCKS_PER_STAGE] = stage2[*i + NUM_BLOCKS_PER_STAGE];
		bwtzip::arith(*stage2[*i + NUM_BLOCKS_PER_STAGE]);
	}
